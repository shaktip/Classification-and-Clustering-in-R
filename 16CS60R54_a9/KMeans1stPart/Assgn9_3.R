...
#For clustering, ignore the date and time features in the data. 
#Also, identify and ignore the
#entries with missing values. Use R to develop the code for k means clustering. 
#First, consider all the
#remaining features for clustering. Then, consider the last three features (7-9) only.
#In each case, vary
...
#To save into a file
png('AllParametersTime.png')
plot(kvalues , tm , type = "l", main = "K V/s Time For All Parameters", xlab = "K of K-Means" , ylab = "Time For Different No of K value")
dev.off()


png('AllParametersAccuracy.png')
plot(kvalues ,accuracy , type = "l", main = "K V/s Accuracy For All Parameters", xlab = "K of K-Means" , ylab = "Accuracy For Different No of K value")

#plot(kvalues , accuracy)
dev.off()
